...
### 4. 局域网访问
1.  **获取本机 IP 地址**：
    -   **Windows**: 打开终端（CMD 或 PowerShell），输入 `ipconfig`，查找 "IPv4 地址"（例如 `192.168.1.100`）。
    -   **macOS/Linux**: 打开终端，输入 `ifconfig` 或 `ip addr`，查找局域网 IP（通常是 `en0` 或 `eth0` 网卡的 `inet` 地址）。

2.  **访问应用**：
    -   **注意**：Docker 部署使用的是 **80** 端口，而不是开发环境的 5173。
    -   在同一局域网内的手机或其他电脑的浏览器中输入：`http://<你的IP地址>`（例如 `http://192.168.1.100` 或 `http://10.62.47.14`）。
    -   如果需要直接访问后端 API，输入：`http://<你的IP地址>:3000`。

3.  **防火墙设置**：
    -   如果无法访问，请检查宿主机的防火墙设置。
    -   **Windows**: 确保防火墙允许 Docker Desktop Backend 或 80 端口的入站连接。
    -   **macOS/Linux**: 确保防火墙（如 `ufw` 或 `iptables`）允许 80 和 3000 端口。
